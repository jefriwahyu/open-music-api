...
const { mapDBaddSong, mapDBgetSongs } = require('../../utils/index');
...
class SongsService {
  ...
  async addSong({
    ...
    if (!result.rows.map(mapDBaddSong)[0].id) {
      throw new InvariantError('Gagal menambahkan lagu');
    }

    return result.rows.map(mapDBaddSong)[0].id;
  }

  async getSongs() {
    const result = await this._pool.query('SELECT id, title, performer, * FROM songs');
    return result.rows.map(mapDBgetSongs);
  }
